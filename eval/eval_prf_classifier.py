...
def main(ckpt, dset_name, root, n_repeat=1, n_proc=10):
    ...
    for (
        ...
        labels = anchor.obj_cat
        _, predicted = torch.max(pred_logits.data, 1)
        train_total += labels.size(0)
        train_correct += (predicted == labels).sum().item()
        train_meta[anchor.obj_cat.item()].append((predicted == labels).sum().item())
    ...
    for (
        ...
        val_meta[anchor.obj_cat.item()].append((predicted == labels).sum().item())

...
from part_embedding.goal_inference.model_sg import segmentation_plot
...
def main(
    ...
    if wandb_log:
        wandb.init(project="4way-segmenter", entity="harryzhangog", config=dict_config)
        run_name = wandb.run.name
        run_name_log = f"4way-segmenter-{run_name}"
        wandb.run.name = run_name_log
    else:
        run_name_log = "debug"

    model = ArtFFDenseSeg().to(device)
    ...
    for i in range(1, n_epochs + 1):
        ...
        if wandb_log:
            wandb.log(
                {
                    "train_rand_plot": segmentation_plot(
                        anchor.pos.reshape((anchor.num_graphs, -1, 3))[0],
                        pred_label=pred_logits.reshape(anchor.num_graphs, -1, 4)[0],
                        gt_label=anchor.seg_label.reshape(
                            -1,
                        )
                        .long()
                        .reshape((anchor.num_graphs, -1, 1))[0],
                    )
                }
            )
        ...
        if wandb_log:
            wandb.log(
                {
                    "val_rand_plot": segmentation_plot(
                        anchor.pos.reshape((anchor.num_graphs, -1, 3))[0],
                        pred_label=pred_logits.reshape(anchor.num_graphs, -1, 4)[0],
                        gt_label=anchor.seg_label.reshape(
                            -1,

...
class SegmenterDataset(tgd.Dataset):
    def __init__(
        self,
        root: str,
        obj_ids: List[str],
    ...
    def processed_dir(self) -> str:
        ...
        if self.even_downsample:
            chunk += "_even"
        return os.path.join(self.root, f"segmentation_wcat" + chunk)
    ...
    def get_data(self, obj_id: str, goal_id) -> ArtData:
        ...
        for curr_idx, start_ang in enumerate([start_ang_obs, start_ang_goal]):
            ...
            if curr_idx == 0:
                segmentation_label = torch.zeros_like(mask)
                segmentation_label[mask == 0] = 0
                segmentation_label[mask == 1] = 1 if currsem == "hinge" else 2
        ...
        anchor_data = tgd.Data(
            ...
            seg_label=segmentation_label.float().reshape(-1, 1),
        )
        demo_data = tgd.Data(
        ...
        for curr_idx, action_pos in enumerate([action_obs_pos, action_goal_pos]):
            ...
            if curr_idx == 0:
                segmentation_label = torch.zeros_like(mask)
                segmentation_label[:500] = 3
                segmentation_label[500:1500] = 0
                segmentation_label[1500:] = 1 if currsem == "hinge" else 2
                return_data.append(P_action_world)
                return_data.append(torch.from_numpy(P_world))
            else:
                return_data.append(torch.from_numpy(P_world))
